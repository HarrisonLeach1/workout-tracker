...
export const useCreateWorkout = (): [
  (values: WorkoutInputs, exercises: Exercise[]) => Promise<void>,
  { loading: boolean; errors: ApolloError[] }
] => {
  const [addWorkout, { loading: addWorkoutLoading, error: addWorkoutError }] = useMutation<
    CreateWorkoutMutation,
    CreateWorkoutMutationVariables
  >(gql(createWorkout));
  ...
  const [addSet, { loading: addSetLoading, error: addSetError }] = useMutation<
    CreateSetMutation,
    CreateSetMutationVariables
  >(gql(createSet));
  ...
  const addExerciseResults = (
    exerciseResultInputs: ExerciseResultInput[],
    createdWorkoutId: string,
    exercises: Exercise[]
  ) => {
    exerciseResultInputs.forEach(async (exerciseResultInput, index) => {
      const createdExerciseResult = await addExerciseResult({
        variables: {
          input: {
            ...exerciseResultInput.createExerciseResultInput,
      ...
      await addSets(exerciseResultInput.createSetInputs, createdExerciseResult.data.createExerciseResult.id);
    });
  };
